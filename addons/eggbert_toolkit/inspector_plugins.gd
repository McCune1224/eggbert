...
## Inspector plugins for CutsceneResource and DialogBranch, plus the
## puzzle cross-reference section. Ported from the old cutscene_inspector
## plugin (unchanged logic, single registration point).
...
const CARDS := preload("res://addons/eggbert_toolkit/cards.gd")


static func create_all() -> Array:
	return [CutsceneResourceInspector.new(), DialogBranchInspector.new(), PuzzleCrossRefInspector.new()]
...
class CutsceneResourceInspector extends EditorInspectorPlugin:
	...
	func _parse_property(_object: Object, _type: int, name: String, _hint_type: int, _hint_string: String, _usage_flags: int, _wide: bool) -> bool:
		return name == "Steps"

	func _parse_begin(object: Object) -> void:
		var helper = CARDS.new()
...
class PuzzleCrossRefInspector extends EditorInspectorPlugin:
	...
	func _parse_begin(object: Object) -> void:
		...
		var links: Array = CARDS.build_cross_refs(object)
		if links.is_empty():
			var note := Label.new()
