...
public partial class OverworldMenu : CanvasLayer
{
  ...
  // WARNING: Deprecated, main menu is very redundant since there are no multiple saves. Quit is sufficient
	...
	public override void _Ready()
	{
		...
		_quitButton = GetNode<Button>("MainPanel/VBoxContainer/GridRow3/QuitButton");
	...
	private void RefreshWarpList()
	{
		...
		foreach (var warp in unlocked)
		{
			...
			string targetName = warp.TargetTransitionName;
			btn.Pressed += () => WarpTo(levelPath, targetName);
			_warpGrid.AddChild(btn);
		}
	}

	private void WarpTo(string levelPath, string targetTransitionName)
	{
		Resume();
		GameController.Instance.LoadLevel(levelPath, targetTransitionName);
		GameLogger.Info("OverworldMenu", $"Warp to: '{levelPath}' at transition '{targetTransitionName}'");
	}
