class_name SunnysideLeaderArena
extends CombatArena

const ENEMY_SCENE: PackedScene = preload("res://combat/enemies/CombatSunnysideLeader.tscn")
const WALL_THICKNESS: float = 16.0
const WALLS_LAYER: int = 2

@export var arena_size: Vector2 = Vector2(540, 360)
@export var enemy_spawn: Vector2 = Vector2(0, -140)

func _ready() -> void:
	player_spawn_position = Vector2(0, 120)
	super._ready()
	_build_bounds(arena_size)
	var enemy: Node = ENEMY_SCENE.instantiate()
	enemy.position = enemy_spawn
	add_child(enemy)
	register_enemy(enemy, "Sunnyside Leader")
	if combat_hud != null:
