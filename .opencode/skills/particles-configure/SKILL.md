...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/particles-configure \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/particles-configure -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
