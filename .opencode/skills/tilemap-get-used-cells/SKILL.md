...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-get-used-cells \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-get-used-cells -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
