...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/gridmap-create \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/gridmap-create -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
