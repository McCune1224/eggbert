...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/particles-set-emitting \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/particles-set-emitting -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
