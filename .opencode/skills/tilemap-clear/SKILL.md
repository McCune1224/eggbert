...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-clear \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-clear -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
