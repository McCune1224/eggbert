...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/gridmap-set-mesh-library \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/gridmap-set-mesh-library -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
