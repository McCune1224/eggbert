...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/gridmap-clear \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/gridmap-clear -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
