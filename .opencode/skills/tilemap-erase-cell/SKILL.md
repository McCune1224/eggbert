...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-erase-cell \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-erase-cell -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
