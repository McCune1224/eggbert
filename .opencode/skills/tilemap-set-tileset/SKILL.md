...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-set-tileset \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-set-tileset -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
