...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/particles-defaults \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/particles-defaults -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
