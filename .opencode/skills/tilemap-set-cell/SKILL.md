...
```bash
curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-set-cell \
...
> Or pipe via stdin:
> ```bash
> curl -X POST https://ai-game.dev/mcp/api/tools/tilemap-set-cell -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
