...
# Each process instantiates one C# scene to avoid a Godot Mono shutdown abort
# observed when multiple unparented C# PackedScene instances are retained.
...
func _assert_level_root(root: Node, scene_name: String) -> void:
	...
	_assert(_script_path(root).ends_with("levels/BaseLevel.cs"), "%s root uses BaseLevel.cs" % scene_name)
	...
	_assert(_script_path(tilemap).ends_with("components/core/LevelTileMapLayer.cs"), "%s CoreTilemapLayer uses LevelTileMapLayer.cs" % scene_name)
...
func _assert_transition(root: Node, node_name: String, expected_level: String, expected_target: String, expected_flag: String, scenes: Dictionary) -> void:
	...
	if transition == null:
		return
	_assert(_script_path(transition).ends_with("levels/LevelTransition.cs"), "%s/%s uses LevelTransition.cs" % [root.name, node_name])
	_assert(transition.get("Level") == expected_level, "%s/%s targets %s" % [root.name, node_name, expected_level])
	_assert(transition.get("TargetTransitionName") == expected_target, "%s/%s targets node %s" % [root.name, node_name, expected_target])
	_assert(transition.get("RequiredFlag") == expected_flag, "%s/%s requires flag '%s'" % [root.name, node_name, expected_flag])
...
func _verify_assembly_line(root: Node) -> void:
	_assert_script(root, "AssemblyLineSavePoint", "saves/SavePoint.cs")
	var assembly_save := root.get_node_or_null("AssemblyLineSavePoint")
	_assert(assembly_save != null and assembly_save.get("LocationName") == "Factory — Assembly Line", "AssemblyLine save point location")
	_assert_script(root, "ShutdownChecklist", "components/npcs/CutsceneTrigger.cs")
	_assert_string_array_contains(root, "ShutdownChecklist", "SetFlagsOnFire", "factory_shutdown_checklist_signed")
	_assert_one_shot(root, "ShutdownChecklist", "shutdown_checklist")
	_assert_script(root, "ConveyorInstruction", "components/npcs/ReadableObject.cs")
	_assert_script(root, "MaintenanceTransferInstruction", "components/npcs/ReadableObject.cs")

	for index in range(1, 13):
		var conveyor_name := "Conveyor%02d" % index
		_assert_script(root, conveyor_name, "components/puzzles/ConveyorTile.cs")
		var conveyor := root.get_node_or_null(conveyor_name)
		if conveyor != null:
			_assert(conveyor.get("ConveyorDirection") == Vector2(-1, 0), "%s pushes west" % conveyor_name)
			_assert(is_equal_approx(float(conveyor.get("ConveyorSpeed")), 80.0), "%s speed is 80" % conveyor_name)

	_assert_script(root, "MaintenancePadWest", "components/puzzles/TeleportPad.cs")
	_assert_script(root, "MaintenancePadEast", "components/puzzles/TeleportPad.cs")
	...
	if west != null and east != null:
		_assert(west.get_node_or_null(west.get("TargetPadPath")) == east, "MaintenancePadWest pairs with MaintenancePadEast")
		_assert(east.get_node_or_null(east.get("TargetPadPath")) == west, "MaintenancePadEast pairs with MaintenancePadWest")

func _verify_control_room(root: Node) -> void:
	_assert_script(root, "ControlRoomSavePoint", "saves/SavePoint.cs")
	var control_save := root.get_node_or_null("ControlRoomSavePoint")
	_assert(control_save != null and control_save.get("LocationName") == "Factory — Control Room", "ControlRoom save point location")
	_assert_script(root, "InspectionDoor", "components/puzzles/Door.cs")
	_assert_script(root, "InspectionApproved", "components/npcs/CutsceneTrigger.cs")
	_assert_string_array_contains(root, "InspectionApproved", "SetFlagsOnFire", "factory_shutdown_inspection_complete")
	_assert_one_shot(root, "InspectionApproved", "shutdown_inspection_complete")
	_assert_script(root, "SequenceController", "components/puzzles/SequencePuzzleController.cs")
	_assert_script(root, "SequenceInstruction", "components/npcs/ReadableObject.cs")

	for index in range(3):
		var plate_name := "SequencePlate%c" % char(65 + index)
		_assert_script(root, plate_name, "components/puzzles/SequencePressurePlate.cs")
		var plate := root.get_node_or_null(plate_name)
		if plate != null:
			_assert(int(plate.get("SequenceIndex")) == index, "%s SequenceIndex is %d" % [plate_name, index])

	var controller := root.get_node_or_null("SequenceController")
	if controller != null:
		var plate_paths = controller.get("PlatePaths")
		...
		var door_path = controller.get("TargetDoorPath")
		...
		_assert(is_equal_approx(float(controller.get("TimeWindow")), 5.0), "SequenceController time window is 5 seconds")

	_assert_script(root, "EggdropSoupPickup", "components/items/PickupItem.cs")
	var pickup := root.get_node_or_null("EggdropSoupPickup")
	if pickup != null:
		_assert(pickup.get("ItemId") == "eggdrop_soup", "EggdropSoupPickup item id is eggdrop_soup")
		_assert(int(pickup.get("Count")) == 1, "EggdropSoupPickup count is 1")
...
func _assert_one_shot(root: Node, node_path: String, expected_id: String) -> void:
	...
	_assert(node.get("CutsceneId") == expected_id, "%s/%s CutsceneId is %s" % [root.name, node_path, expected_id])
